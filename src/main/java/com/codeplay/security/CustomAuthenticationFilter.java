...
public class CustomAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
    ...
    private UsernamePasswordAuthenticationToken getAuthRequest(HttpServletRequest request) throws Exception {
        try {
            ...
            log.debug("1.CustomAuthenticationFilter :: userId:" + user.getUser_email() + " userPw:" + user.getUser_password());

            // ID와 패스워드를 기반으로 토큰 발급
            return new UsernamePasswordAuthenticationToken(user.getUser_email(), user.getUser_password());
